...
function logAndExit(message = '', color = null) {
    log(message, color);
    process.exit(0);
}
...
function copyToPackage(from, to) {
    if (to.startsWith('.')) {
        to = pathHelper_1.default.buildRelative(to);
    }
    fs.copySync(from, to);
}
...
function commandExists(command) {
    return command_exists_1.sync(command);
}
...
function logOperation(title, operation) {
